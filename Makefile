...
$(BUILDS_DIR)/cpu: $(SRC_DIR)/cpu $(BUILDS_DIR)
	cd $(SRC_DIR)/cpu && go build -o $(BUILDS_DIR)/cpu $(SRC_DIR)/cpu/cmds/cpu/.

$(BUILDS_DIR)/cpud: $(SRC_DIR)/cpu $(BUILDS_DIR)	
	cd $(SRC_DIR)/cpu && go build -o $(BUILDS_DIR)/cpud $(SRC_DIR)/cpu/cmds/cpud/.
...
$(BUILDS_DIR)/initramfs.cpio: $(BUILDS_DIR)/u-root/u-root $(BUILDS_DIR)/identity.pub $(SRC_DIR)/go.work
	$(BUILDS_DIR)/u-root/u-root -o $(BUILDS_DIR)/initramfs.cpio -files ${BUILDS_DIR}/identity.pub:key.pub -files /mnt ./u-root/cmds/core/init ./u-root/cmds/core/gosh ./cpu/cmds/cpud

.PHONY: build
build: $(BUILDS_DIR)/initramfs.cpio $(BUILDS_DIR)/cpu $(BUILDS_DIR)/cpud
